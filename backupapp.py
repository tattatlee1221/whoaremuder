...
def call_siliconflow_api(prompt, config, max_chars=None):
    ...
    try:
       ...
       # if max_chars and len(result) > max_chars:
            #result = result[:max_chars] + "..."  # 截斷超長回應
...
def init_game():
    ...
    兇手是{killer}。請為每個角色生成一個相關線索（約20字），並描述案件有趣的經過（包含背景事件，例如案發前的情況）。
    ...
    if story_raw is None or not story_raw.strip():
        ...
        try:
            ...
            game_data = {
                "case": case_data,
                "roles": {role: {**data, **role_data[role]} for role, data in roles_dict.items()}
            }
...
def talk_to_role():
    ...
    if role_data["is_killer"]:
        ...
        你的線索：{role_data['clue']}。
        玩家問：'{question}'，請用你的性格回應，試圖誤導玩家，但保持合理性。回應不超過250字。如果玩家猜到你了。你要說明犯案原因啊。
        請用繁體中文回應。
        """
    elif is_victim:
        prompt = f"""
        ...
        prompt = f"""
        我們來玩一個猜猜兇手的角色扮演遊戲。你是{role}，性格{role_data['personality']}，不是兇手。
        案件背景：{case_data['case_type']}發生在{case_data['location']}，時間是{case_data['time']}，受害者是{case_data['victim']}。
        背景事件：{case_data['events']}。
        你的線索：{role_data['clue']}。
